...
func ExampleReject() {
	...
	fmt.Println(slices.Reject(cond, ss))
	// Output:
	// [foo]
}
...
func TestReject(tt *testing.T) {
	...
	tt.Run("str", func(t *testing.T) {
		...
		test := func(input, expected []string) {
			...
			if actual := slices.Reject(isFoo, input); !equal(actual, expected) || same(actual, input) {
				t.Errorf("Reject(`%#v`)=`%#v`, expected `%#v`", input, actual, expected)
			}

			input2 := slices.Clone(input)
			if actual := slices.RejectInPlace(isFoo, input2); !equal(actual, expected) {
